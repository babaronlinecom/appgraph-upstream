...
/**
 * Mermaid ER diagram built strictly from parsed schema facts (data_model nodes
 * and `references` edges). Returns null when the repository has no data graph.
 */
export function toMermaidErd(graph: AppGraphDocument): string | null {
  const models = graph.nodes.filter((node) => node.type === "data_model");
  if (models.length === 0) return null;
  const nodeById = new Map(graph.nodes.map((node) => [node.id, node]));
  const safeId = (label: string) => label.replace(/[^A-Za-z0-9_]/g, "_") || "model";

  const lines: string[] = ["erDiagram"];

  for (const model of models) {
    const fields = Array.isArray(model.metadata.fields)
      ? (model.metadata.fields as Array<{
          name: string;
          type: string;
          primaryKey?: boolean;
          unique?: boolean;
      ...
      : [];
    lines.push(`  ${safeId(model.label)} {`);
    for (const field of fields.slice(0, 20)) {
      const type = field.type.replace(/[^A-Za-z0-9_[\]]/g, "") || "string";
      const keys = [
        field.primaryKey ? "PK" : "",
        field.relation ? "FK" : "",
        field.unique ? "UK" : "",
        ...
        .filter(Boolean)
        .join(",");
      lines.push(`    ${type} ${field.name}${keys ? ` ${keys}` : ""}`);
    }
    lines.push("  }");
  }

  for (const node of graph.nodes) {
    if (node.type !== "data_enum") continue;
    const values = Array.isArray(node.metadata.values) ? (node.metadata.values as string[]) : [];
    lines.push(`  %% enum ${node.label}: ${values.join(", ")}`);
  }
